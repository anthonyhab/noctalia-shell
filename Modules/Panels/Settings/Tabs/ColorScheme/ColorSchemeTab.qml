...
import qs.Widgets
 ...
 ColumnLayout {
  ...
  NToggle {
    ...
    visible: !Settings.data.omarchy.active
    enabled: !Settings.data.omarchy.active
    checked: false
    onVisibleChanged: {
      if (visible) {
        checked = false
        // Re-check availability when toggle becomes visible
        OmarchyService.checkAvailability()
      }
    }
    onToggled: checked => {
                 if (checked) {
  ...
  NBox {
    ...
    ColumnLayout {
      ...
      // Custom theme-set command
      NTextInput {
        label: I18n.tr("settings.color-scheme.omarchy.active.custom-command.label")
        description: I18n.tr("settings.color-scheme.omarchy.active.custom-command.description")
        placeholderText: OmarchyService.omarchyThemeSetPathDefault
        text: Settings.data.omarchy.themeSetCommand
        Layout.fillWidth: true
        Layout.topMargin: Style.marginM
        onEditingFinished: {
          Settings.data.omarchy.themeSetCommand = text
          Logger.i("ColorSchemeTab", "Custom Omarchy theme-set command set to:", text || "(default)")
        }
      }
