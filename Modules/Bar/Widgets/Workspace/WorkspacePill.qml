...
Item {
  ...
  readonly property int pillMainExtent: isExpanded
      ? WorkspaceMetrics.pillMainLength(labelMainExtent, visibleWindows.length, overflowCount, panelSlotExtent, panelGap, panelLeadingInset, panelTrailingInset, revealInset)
      : Math.max(capsuleHeight, labelMainExtent)
  ...
  readonly property real revealPanelInactiveIconOpacity: isFocused
      ...
      : Math.min(unfocusedIconsOpacity, 0.68)
  ...
  Connections {
    target: root.workspaceModel && root.workspaceModel.windows && root.workspaceModel.windows.count !== undefined
        ? root.workspaceModel.windows
        : null
