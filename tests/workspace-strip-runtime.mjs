...
const harnessPath = path.join(repoRoot, "tests/qml/WorkspaceStripRuntimeHarness.qml");
...
const configDir = fs.mkdtempSync(path.join(os.tmpdir(), "workspace-strip-runtime-config-"));
const runtimeDir = fs.mkdtempSync(path.join(os.tmpdir(), "workspace-strip-runtime-"));
...
ensureDir("tests/qml");
fs.copyFileSync(harnessPath, path.join(configDir, "tests/qml/WorkspaceStripRuntimeHarness.qml"));
...
ShellRoot {
  RuntimeHarness.WorkspaceStripRuntimeHarness {}
}
`);
...
const requiredMarkers = [
  "WORKSPACE_STRIP_RUNTIME_PASS strip_has_four_pills",
  "WORKSPACE_STRIP_RUNTIME_PASS resting_pills_share_same_width",
  "WORKSPACE_STRIP_RUNTIME_PASS resting_pills_share_same_width_with_scratchpad",
  "WORKSPACE_STRIP_RUNTIME_PASS active_drawer_stays_open",
  "WORKSPACE_STRIP_RUNTIME_PASS hovered_occupied_inactive_expands",
  "WORKSPACE_STRIP_RUNTIME_PASS hover_preview_coexists_with_active_drawer",
  "WORKSPACE_STRIP_RUNTIME_PASS trailing_only_keeps_leading_positions",
  "WORKSPACE_STRIP_RUNTIME_PASS trailing_only_keeps_active_position",
  "WORKSPACE_STRIP_RUNTIME_PASS trailing_only_keeps_hovered_pill_position",
  "WORKSPACE_STRIP_RUNTIME_PASS trailing_only_shifts_following_pills",
  "WORKSPACE_STRIP_RUNTIME_PASS hover_ownership_transfers_without_collapse_jitter",
  "WORKSPACE_STRIP_RUNTIME_PASS hover_transfer_keeps_expanded_width",
  "WORKSPACE_STRIP_RUNTIME_PASS hover_release_collapses_preview",
  "WORKSPACE_STRIP_RUNTIME_PASS hover_release_restores_resting_width",
  "WORKSPACE_STRIP_RUNTIME_PASS active_width_stays_stable",
  "WORKSPACE_STRIP_RUNTIME_RESULT PASS",
];

if (output.includes("WORKSPACE_STRIP_RUNTIME_FAIL")) {
...
if (result.error && !(result.error.code === "EPERM" && output.includes("WORKSPACE_STRIP_RUNTIME_RESULT PASS"))) {
