...
Singleton {
  ...
  readonly property string omarchyBackgroundPath: omarchyConfigDir + "current/background"
  readonly property string omarchyThemesDir: omarchyConfigDir + "themes"
  readonly property string omarchyThemeSetPathDefault: {
  ...
  readonly property string omarchyThemeSetPath: {
    // Use custom command if set, otherwise fall back to default
    const custom = Settings.data.omarchy.themeSetCommand?.trim() || ""
    if (custom !== "") {
      // Expand ~ to home directory
      if (custom.startsWith("~/")) {
        const configDir = Settings.configDir
        const home = configDir.replace(/\/.config\/.*$/, '')
        return home + custom.slice(1)
      }
      return custom
    }
    return omarchyThemeSetPathDefault
  }
  readonly property string outputJsonPath: Settings.configDir + "omarchy-colors.json"
  property bool available: false
  property string themeName: ""
  property var availableThemes: []
  property bool loadFailureHandled: false
  property bool isInitialCheck: false
  ...
  function activate() {
    ...
    if (!available) {
      ...
      ToastService.showError(I18n.tr("toast.omarchy.title"), I18n.tr("toast.omarchy.config-not-found"))
      return false
    }

    // Reset failure flag for new activation attempt
  ...
  function reload() {
    ...
    // Reset failure flag for this reload attempt
    loadFailureHandled = false
  ...
  function setTheme(themeName) {
    if (!available) {
      ...
      ToastService.showError(I18n.tr("toast.omarchy.title"), I18n.tr("toast.omarchy.not-available"))
      return false
    }
    ...
    if (!themeExists) {
      ...
      ToastService.showError(I18n.tr("toast.omarchy.title"), I18n.tr("toast.omarchy.theme-not-found", { "theme": themeName }))
      return false
    }

    Logger.i("Omarchy", "Setting theme to:", themeName)
    Logger.i("Omarchy", "Using command:", omarchyThemeSetPath)
  ...
  Process {
    ...
    onExited: function (code) {
      ...
      if (root.available) {
        ...
        // Only auto-reload on initial startup check, not on every availability check
        if (root.isInitialCheck && Settings.data.omarchy.active) {
      ...
      // Reset flag after initial check
      root.isInitialCheck = false
    }
  }
  ...
  FileView {
    ...
    onLoadFailed: function (error) {
      ...
      // Only handle the failure once per activation attempt to prevent spam
      if (!root.loadFailureHandled) {
        root.loadFailureHandled = true
        Logger.w("Omarchy", "Omarchy config not available --", error.toString ? error.toString() : error)
        ToastService.showError(I18n.tr("toast.omarchy.title"), I18n.tr("toast.omarchy.config-not-found"))

        // Auto-disable if it was enabled
        if (Settings.data.omarchy.active) {
          Logger.i("Omarchy", "Auto-disabling Omarchy due to missing config")
          Settings.data.omarchy.active = false
        }
      }
    }
  ...
  Process {
    ...
    onExited: function (code) {
      if (code === 0) {
        ...
        ToastService.showNotice(I18n.tr("settings.color-scheme.title"), I18n.tr("toast.omarchy.theme-applied", { "theme": displayName }), "settings-color-scheme")
        if (!Settings.data.omarchy.active) {
          Settings.data.omarchy.active = true
        }
        root.schemeReady()
        ...
        ToastService.showNotice(I18n.tr("settings.color-scheme.title"), I18n.tr("toast.omarchy.failed-to-load"), "settings-color-scheme")
      }
    }
  ...
  Process {
    ...
    onExited: function (code) {
      if (code === 0) {
        ...
        ToastService.showError(I18n.tr("toast.omarchy.title"), I18n.tr("toast.omarchy.failed-to-switch"))
      }
    }
